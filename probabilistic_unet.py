...
# CPU 과소비 하지 않게 설정 (실험 여러개 돌릴 수 있게 )
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels, num_filters, no_convs_per_block, initializers, padding=True, posterior=False):
        ...
        if posterior:
            ...
            self.input_channels += 3 # segm 3으로 수정!
...
class Fcomb(nn.Module):
    ...
    and output of the UNet (the feature map) by concatenating them along their channel axis. # channel 축을 따라 concatenate함. UNet의 output과 latent space에서 취한 sample을 결함하는...  1x1 conv layers* 
    """
    def __init__(self, num_filters, latent_dim, num_output_channels, num_classes, no_convs_fcomb, initializers, use_tile=True):
        ...
        self.verbose = True # On/Off of print! # 230110
...
class ProbabilisticUnet(nn.Module):
    ...
    def __init__(self, input_channels=1, num_classes=1, num_filters=[32,64,128,192], latent_dim=6, no_convs_fcomb=4, beta=10.0):
        ...
        self.posterior = AxisAlignedConvGaussian(self.input_channels, self.num_filters, self.no_convs_per_block, self.latent_dim, self.initializers, posterior=True) # .to(device) # Posterior option여부만 True, False
    ...
    #     criterion = nn.BCEWithLogitsLoss(size_average = False, reduce = False,
    #     reduction=None)
    #     z_posterior = self.posterior_latent_space.rsample()
...
class ProbabilisticUnet_3output(nn.Module):
    """
    A probabilistic UNet (https://arxiv.org/abs/1806.05034) implementation.
    input_channels: the number of channels in the image (1 for greyscale and 3 for RGB)
    num_classes: the number of classes to predict
    ...
    def __init__(self, input_channels=1, num_classes=1, num_filters=[32,64,128,192], latent_dim=6, no_convs_fcomb=4, beta=10.0):
        ...
        self.unet = Unet(self.input_channels, self.num_classes, self.num_filters, self.initializers, apply_last_layer=True, padding=True) # .to(device)
